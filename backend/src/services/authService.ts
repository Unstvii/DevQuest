...
class authService {
    ...
    createToken = async (user: User): Promise<{ accessToken: string, refreshToken: string }> => {

        const payloadShort = {
            id: user.id
        }
        const payload = {
            username: user.username, xp: user.xp, level: user.level, email: user.email
        }
        const accessToken = jwt.sign(payloadShort, this.ACCESS_SECRET, { expiresIn: "15m" })
